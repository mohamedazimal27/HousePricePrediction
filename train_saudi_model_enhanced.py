#!/usr/bin/env python3
"""
Enhanced training script for Saudi housing data with improved accuracy
"""
...
print("=== ENHANCED SAUDI HOUSING MODEL ===")
...
for name, model in models.items():
    ...
    print(f"{name}:")
    print(f"  CV R²: {scores.mean():.4f} (+/- {scores.std() * 2:.4f})")
...
plt.title('Residual Plot - Enhanced Model')
plt.tight_layout()
plt.savefig('residual_plot_enhanced.png')
plt.close()

# 3. Actual vs Predicted with confidence interval
...
margin = stats.t.ppf((1 + confidence) / 2, len(errors) - 1) * np.std(errors)
...
print("- residual_plot_enhanced.png")
